...
uint8_t broadcastAddress[] = {0x5C, 0xCF, 0x7F, 0x0E, 0x27, 0xB5};
...
int servoPin = D0;            //定义GPIO16 D0用于控制舵机
int D_Button = D2;            //定义GPIO4 D2用于触摸开关控制舵机开门
int L_Button = D8;            //定义GPIO15 D8用于触摸开关控制开关灯
...
void setup()
{
  ...
  digitalWrite(servoPin, LOW);  //初始化舵机输出端口低电平
...
void loop()
{
  ...
  if (digitalRead(D_Button) == 1)  //未触摸开关时高电平，值为1
  {
    DOOR = 0; 
  }
  else
  ...
  if (digitalRead(L_Button) == 0)  //触摸开关时低电平，值为0
  {
    Light = 0;
    myData.b = Light;
    // Send message via ESP-NOW
